...
This program's main function contains the code to time the diffusion method, which contains the diffusion code.
This method asks for user input on the size of the cube, and uses an if statement to control the Msize (will print "invalid input" if you enter 
a number outside the bounds (set between 0 and 100, but can be changed at line 8)), with a while and an if statement which navigates the user to the partition code or the non-partition code until they have entered the correct input and "invalid input" is printed on the screen.
...
The code will prompt you with: "Please enter 'y' for partition, and 'n' to run without a partition: "
Type: n
...
Lastly, the program will print "Wall Time: (time) seconds" and give the user the wall time.
